...
jQuery(function () {
  const form$ = jQuery("form#companyContactStatusModal");
  const create$ = jQuery("button.btn-company-contact-status-create");
  const modal$ = bootstrap.Modal.getOrCreateInstance(form$[0], {});
  const table$ = jQuery("#companyContactStatusTable");
  let tableInstance$;
  ...
  table$.on("click", ".btn-delete", async function () {
    ...
    const options = {
      url:`company-contact-status/${slug}`,
      method:"DELETE",
      success:function(){
        getCompanyStatus();
      }
