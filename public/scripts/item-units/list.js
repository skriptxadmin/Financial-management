...
jQuery(function () {
  ...
  const create$ = jQuery("button.btn-item-unit-create");
  ...
  function getItemUnits() {
    ...
    const options = {
      ...
      success: function (response) {
        drawDatatable(response.itemUnits);
      },
    };
    ajax(options);
  }
  ...
  table$.on("click", ".btn-delete", async function () {
    const result = await ratify("Are you sure you want to delete this item unit?");
    if (!result) return;
