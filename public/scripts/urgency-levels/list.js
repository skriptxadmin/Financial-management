...
jQuery(function () {
  const form$ = jQuery("form#urgencyLevelModal");
  const create$ = jQuery("button.btn-urgency-level-create");
  const modal$ = bootstrap.Modal.getOrCreateInstance(form$[0], {});
  const table$ = jQuery("#urgencyLevelsTable");
  let tableInstance$;
  ...
  function getUrgencyLevels() {
    ...
    const options = {
      ...
      success: function (response) {
        drawDatatable(response.urgencyLevels);
      },
    };
    ajax(options);
  }
  ...
  table$.on("click", ".btn-delete", async function () {
    const result = await ratify("Are you sure you want to delete this urgency level?");
    if (!result) return;
    const slug = jQuery(this).closest("tr").attr("data-slug");
    const options = {
      url:`urgency-levels/${slug}`,
      method:"DELETE",
      success:function(){
        getUrgencyLevels();
      }
