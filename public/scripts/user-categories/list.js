...
jQuery(function () {
  const form$ = jQuery("form#userCategoryModal");
  const create$ = jQuery("button.btn-user-category-create");
  const modal$ = bootstrap.Modal.getOrCreateInstance(form$[0], {});
  const table$ = jQuery("#userCategoriesTable");
  let tableInstance$;
  ...
  table$.on("click", ".btn-delete", async function () {
    const result = await ratify("Are you sure you want to delete this user categories?");
    if (!result) return;
    const slug = jQuery(this).closest("tr").attr("data-slug");
    const options = {
      url:`user-categories/${slug}`,
      method:"DELETE",
      success:function(){
        getUserCategories();
      }
